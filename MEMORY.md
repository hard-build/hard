...
On 2026-09-16 the user selected `7.0` as the next development version.
`make bump VERSION=7.0` advanced the embedded number from `6.0` while keeping
the `development` prerelease identifier. Historical container-image tags and
test fixtures remain unchanged. The source bump was subsequently committed and
